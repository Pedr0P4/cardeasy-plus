...
public class TeamsService {
  ...
  public Team findById(UUID id) {
    return this.teams.findById(id).orElseThrow(TeamNotFound::new);
  }

  public Team findByCode(String code) {
    return this.teams.findByCode(code).orElseThrow(TeamNotFound::new);
  }
  ...
  public Team update(UUID id, String title, String description) {
  ...
  public Team join(UUID accountId, String code) {
    Account account = this.accounts.findById(accountId).orElseThrow(
      AccountNotFound::new
    );
  ...
  public void kick(UUID accountId, UUID teamId) {
    ...
    if (participation.getRole() == Role.OWNER) throw new CannotKickOnwer();

    this.participations.deleteById(participationId);
  }

  public void existsById(UUID id) {
    if (!this.teams.existsById(id)) throw new TeamNotFound();
  }
}
