import clsx from "clsx";
import { notFound } from "next/navigation";
import CloseButtonParallelRoute from "@/components/CloseButtonParallelRoute";
import EditAttachmentFormSection from "@/components/projects/forms/modals/EditAttachmentFormSection";
import { Api } from "@/services/api";

export default async function CreateAttachmentModal({
  params,
}: Readonly<{
  params: Promise<{
    project: string;
    cardList: string;
    card: string;
    attachment: string;
  }>;
}>) {
  const {
    project: projectId,
    cardList: cardListId,
    card: cardId,
    attachment: attachmentId,
  } = await params;

  const project = await Api.server()
    .projects()
    .get(Number.parseInt(projectId, 10))
    .catch(() => notFound());

  const cardList = await Api.server()
    .cardLists()
    .get(Number.parseInt(cardListId, 10))
    .catch(() => notFound());

  const card = await Api.server()
    .cards()
    .get(Number.parseInt(cardId, 10))
    .catch(() => notFound());

  const attachment = await Api.server()
    .attachments()
    .get(Number.parseInt(attachmentId, 10))
    .catch(() => notFound());

  return (
    <dialog open className="modal">
      <div
        className={clsx(
          "modal-box flex flex-col gap-4 !px-0 !pb-0 !pt-2",
          "scrollbar scrollbar-thin scrollbar-thumb-base-content",
          "scrollbar-track-base-200 relative",
        )}
      >
        <CloseButtonParallelRoute />
        <main className="relative w-full h-full flex flex-1 flex-col">
          <EditAttachmentFormSection
            attachment={attachment}
            cardList={cardList}
            project={project}
            card={card}
          />
        </main>
      </div>
    </dialog>
  );
}
